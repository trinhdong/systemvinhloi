$(document).ready(function () {
    ...
    $(document).on('click', '#updatePayment', function () {
        if (isEdit) {
            if ($('#payment-method').length > 0 && $('select[name="payment_method"]').val() == '') {
            ...
            if (isTranfer || $('#payment-method').length > 0 && $('select[name="payment_method"]').val() == 1) {
                let $isValid = true;
        ...
        $('#update-payment').find('#paid, #datepicker, input[name="bank_name"], input[name="bank_code"], input[name="bank_customer_name"], select[name="bank_account_id"]').remove()
        $(this).prop('disabled',true);
        $(this).closest('#updatePaymentModal').modal('hide');
    })
    ...
    $(document).on('focus', '#approveNote, #rejectNote, #paid, input[name="bank_code"], input[name="bank_name"], input[name="bank_customer_name"], select[name="bank_account_id"]', function () {
