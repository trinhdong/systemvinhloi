$(document).ready(function() {
    ...
    $('.order_number').keyup(function(){
        $('.order_number_paper').text($(this).val());
    })
    $('.order_code').keyup(function(){
        $('.order_code_paper').text($(this).val());
    })
    $(document).on('change','input[name=created_date]',function() {
        date_create = $(this).val()
        $('.create_date_paper').text(date_create)
    })
    $(document).on('change','input[name=order_date]',function() {
        date_paper = $(this).val()
        $('.order_date_paper').text(date_paper)
    })
    $('.place_of_import').keyup(function(){
        $('.place_of_import_paper').text($(this).val());
    })
    $('.slit_number').keyup(function(){
        $('.slit_number_paper').text($(this).val());
    })
    ...
    $('.serial_number').keyup(function(){
        $('.seri-number').text($(this).val());
    })
    $('.created_person').keyup(function(){
        $('.created-person').text($(this).val());
    })
    ...
    $(".btn-close-col-left").click(function() {
        $(".col-left").toggle(100, "linear", function() {
            if ($(".col-left").is(":visible")) {
                $(".btn-close-col-left h5").text("プレビュー拡大 <<").css({
                    "font-size": "20px",
                    "color": "black",
                    "position": "relative",
                    "left": "0px"
    ...
    $('.add-line').click(function(){
        ...
        _html = getHTML_InputConstract(id);
        $('.form-input-supplier-contract').append(_html);
    })
    ...
    // open modal description sub-detail
...
function updateLine(){
    ...
    $('.form-memo').each(function( index ){
        ...
        data = {
            id : $(this).data('id'),
            product_name : $(this).find('input[name="product_name_'+id+'"]').val(),
            //product_id : $(this).find('#product_name_'+id).val(),
...
//function get HTML
function getHTML_InputConstract(id){
    ...
    _html += '            <input type="text" class="form-control padding-input product_code" style="width:50px;">'
    _html += '            <input class="form-control padding-input product_name" name="product_name_'+id+'" type="text" style="width:200px;">'
    _html += '        </div>'
    _html += '    </div>'
    ...
    _html += '    <div class="px-1 py-2 text-center" style="min-width:90px;width:100%;"><input class="form-control padding-input buy_in_discount_rate" type="number" min="1"></div>'
    _html += '    <div class="px-1 py-2 text-center" style="min-width:110px;width:100%;"><input class="form-control padding-input buy_price" type="number" min="1"></div>'

    _html += '    <div class="px-1 py-2 text-center" style="min-width:90px;width:100%;"><input class="form-control padding-input sales_discount_rate" type="number" min="1"></div>'
    _html += '    <div class="px-1 py-2 text-center" style="min-width:110px;width:100%;"><input class="form-control padding-input price" type="number" min="1"></div>'
    ...
    _html += '    <div class="px-1 py-2 text-center" style="min-width:76px;width:100%;"><input class="form-control padding-input tax" type="number" min="1"></div>'
    _html += '    <div class="px-1 py-2 text-center" style="min-width:120px;width:100%;"><input class="form-control padding-input delivery_term_'+id+'" type="text"></div>'
...
function updatePaper2tabFirst(parent_memo,data){
    ...
    //parent_memo.find('.product_id').text(data.product_id)
...
function getManufacturerOfProduct(id){
    $(document).ready(function () {
        $('input[name="product_name_'+id+'"]').autocomplete({
            source: list_product
        });
    })
}
...
function getDataMemoContractDetail(){
    ...
    $('#copy .detail_memo').each(function( index ){
        element = {
            ...
            tax: $(this).find('.tax').text().replace(/,/g, ''),
        ...
        data_contract_detail.push(element);
    })
    return data_contract_detail;
}
