        ...
        <div class="card-body">
            <div class="card">
                <div class="card-header py-3">
                    <div class="row g-5">
                        <div class="col-3">
                            <input name="order_number" type="text" class="form-control" placeholder="Mã đơn hàng" value="{{ $order->order_number }}">
                        ...
                        <div class="col-3">
                            <select id="customer" name="customer_id" class="form-select single-select">
                                ...
                                @foreach($customers as $customerId => $customerName)
                                    <option value="{{ $customerId }}" @if($order->customer_id == $customerId) selected @endif>{{ $customerName }}</option>
                                @endforeach
                            </select>
                ...
                <div class="card-body">
                                ...
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table align-middle mb-0">
                                            ...
                                            <tr class="productOrder" data-id="{{ $orderDetail->product_id }}" id="orderDetail{{$orderDetail->id}}">
                                                <td>
                                                    <a href="javascript:;" id="deleteOrderDetailModalBtn" class="btn btn-sm btn-outline-danger border-0"
                                                       title="Xóa"
                                                       data-bs-toggle="modal"
                                                ...
                                                <td style="min-width:150px">
                                                    <textarea name="note[]" id="" cols="1" rows="1" class="form-control">{{ $orderDetail->note }}</textarea>
                                                </td>
                                                <td>
                                                    <div class="product-price">{{number_format($orderDetail->product_price)}}</div>
                                                    <input type="hidden" name="product_price[]" value="{{$orderDetail->product_price}}"/>
                                                </td>
                                                <td class="discount-percent">
                                                    <div class="input-group has-validation">
                                                        <input name="discount_percent[]" type="text" readonly="" class="disabled form-control"
                                                               placeholder="000.00" autocomplete="off" value="{{ $orderDetail->discount_percent }}">
                                                        <div class="invalid-feedback"></div>
                                                    </div>
                                                </td>
                                                <td>
                                                ...
                                                <td class="quantity">
                                                    <div class="input-group has-validation">
                                                        <input style="max-width: 80px" name="quantity[]" type="text" min="1" step="1" class="w-auto form-control"
                                                               placeholder="0" autocomplete="off" value="{{number_format($orderDetail->quantity)}}">
                                                        <div class="invalid-feedback"></div>
                                                    </div>
                                                </td>
                            ...
                            <div class="card border shadow-none bg-light radius-10">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex align-items-center mb-3">
                                        ...
                                        <div class="ms-auto">
                                            <input type="hidden" name="order_total_product_price" value="{{$order->order_total_product_price}}" />
                                            <h5 id="total-product-order" class="mb-0">{{number_format($order->order_total_product_price)}}₫</h5>
                                        </div>
                                    </div>
                                    <div class="d-flex align-items-center mb-3">
                                        <div>
                                        ...
                                        <div class="ms-auto">
                                            <input type="hidden" name="order_discount" value="{{$order->order_discount}}" />
                                            <h5 id="total-discount"  class="mb-0 text-danger">{{number_format($order->order_discount)}}₫</h5>
                                        </div>
                                    </div>
                                    <div class="d-flex align-items-center mb-3">
                                        <div>
                                        ...
                                        <div class="ms-auto">
                                            ...
                                            <h5 id="total-order"  class="mb-0 text-danger">{{number_format($order->order_total)}}₫</h5>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-8">
                            <div id="delivery-info" class="card border radius-10">
                                <div class="card-body">
                                    ...
                                    <p id="delivery-info-name"><strong>Tên: </strong><span>{{$order->customer->name}}</span></p>
                                    <p id="delivery-info-address"><strong>Địa chỉ: </strong><span>{{$order->customer->address}}</span></p>
                                    <p id="delivery-info-phone"><strong>Số điện thoại: </strong><span>{{$order->customer->phone}}</span></p>
                                    <div class="align-items-center">
                                        ...
                                        <input name="shipping_address" type="text" class="form-control"
                                               placeholder="Nhập địa chỉ giao hàng" autocomplete="off" value="{{$order->shipping_address}}">
                                        <div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-4">
                            <div id="payment-info" class="card border radius-10">
                                <div class="card-body">
                                    ...
                                    <div class="mb-3">
                                        <select id="payment-type" name="payment_type" class="form-select">
                                            ...
                                            @foreach(PAYMENTS_TYPE as $k => $v)
                                                <option value="{{ $k }}" @if($order->payment_type == $k) selected @endif>{{ $v }}</option>
                                            @endforeach
                                        </select>
                                        <div class="invalid-feedback">Vui lòng nhập hình thức thanh toán</div>
                                    </div>
                                    <div>
                                        <select id="payment-method" name="payment_method" class="form-select {{$order->payment_type == 3 ? 'd-none' : ''}}">
                                            <option selected="" value="">Phương thức thanh toán</option>
                                            @foreach(PAYMENTS_METHOD as $k => $v)
                                                <option value="{{ $k }}" @if($order->payment_method == $k) selected @endif>{{ $v }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div id="payment-method-info" class="{{$order->payment_method == 1 ? '' : 'd-none'}}">
                                        <div class="align-items-center mt-3">
                                            ...
                                            <input name="bank_customer_name" type="text" class="form-control"
                                                   placeholder="Nhập tên chủ tài khoản" autocomplete="off" value="{{$order->bank_customer_name}}">
                                        ...
                                        <div class="align-items-center mt-3">
                                            ...
                                            <input name="bank_name" type="text" class="form-control"
                                                   placeholder="Nhập tên ngân hàng" autocomplete="off" value="{{$order->bank_name}}">
                                        ...
                                        <div class="align-items-center mt-3">
                                            ...
                                            <input name="bank_code" type="text" class="form-control"
                                                   placeholder="Nhập số tài khoản" autocomplete="off" value="{{$order->bank_code}}">
                                    ...
                                    <div id="deposit" class="{{$order->payment_type == 2 ? '' : 'd-none'}} align-items-center mt-3">
                                        <label for="" class="fw-bolder me-1" style="white-space: nowrap">Số tiền cọc: </label>
                                        <input name="deposit" type="text" class="form-control"
                                               placeholder="Nhập số tiền cọc" autocomplete="off"  value="{{$order->deposit > 0 ? number_format($order->deposit) : ''}}">
                        ...
                        <div class="col-8">
                            <div id="order-note" class="card border radius-10">
                                <div class="card-body">
                                    ...
                                    <div>
                                        <textarea name="order_note" class="form-control" id="" cols="30" rows="2">{{$order->order_note}}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div id="red-bill" class="{{$order->customer->tax_code != null && $order->customer->email != null && $order->customer->company != null ? '' : 'd-none'}} col-2">
                        <div class="form-check">
                            <input {{$order->is_print_red_invoice == 1 ? 'checked' : ''}} class="form-check-input" value="1" type="checkbox" id="gridCheck1" name="is_print_red_invoice">
                    ...
                    <div class="col-8">
                        <div id="red-bill-info" class="{{$order->is_print_red_invoice == 1 ? '' : 'd-none'}} card border radius-10">
                            <div class="card-body">
                                <div class="d-flex align-items-center mb-4">
                                    <div>
                                ...
                                <p id="red-bill-info-company"><strong>Tên công ty: </strong><span>{{$order->customer->company}}</span></p>
                                <p id="red-bill-info-tax_code"><strong>Mã số thuế: </strong><span>{{$order->customer->tax_code}}</span></p>
                                <p id="red-bill-info-email"><strong>Email: </strong><span>{{$order->customer->email}}</span></p>
                            </div>
                        </div>
                    </div>
        </div>
