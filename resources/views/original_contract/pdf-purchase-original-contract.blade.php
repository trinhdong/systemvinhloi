...
<body >
<div class="container" >
    <div class="title text-dark-blue">{{$title}}</div>
    <div class="info-pdf" >
        <div class="info-pdf-left" >
            <table class="info-table-border" style="margin-bottom:0px;border-collapse: collapse;word-wrap:break-word;">
                ...
                <tr>
                    ...
                    <td style="width:70%;" colspan="3">{{$data->Customer ? $data->Customer->address : ''}}</td>
                </tr>
                <tr>
        ...
        <div class="info-pdf-center" >
            <div class="" style="margin-bottom:0px;">
                ...
                <div style="line-height:4px;margin-top:5px;margin-left:0px;">
                    ...
                    <div style="margin-top:10px">登録番号 T1010401030016</div>
                </div>
            </div>
    ...
    @if($order_check == false)
        <div class="" style="margin-top:200px;border:1px solid white;font-size:12px">
            <div style="display:inline-block;font-size:16px;">{{__('admin.preview_estimated_amount')}} <span style="">{{number_format($data->getRevenue())}}</span>円</div>
            <div style="display:inline-block;color:rgb(43, 42, 42);opacity:90%">（{{__('admin.preview_internal_consumption_tax')}}<span style="">{{number_format($data->getRevenueNoTax())}}</span>円）</div>
        </div>
    @else 
        <div class="" style="margin-top:200px;border:1px solid white;font-size:12px">
        </div>
    @endif
    <div style="background-color:lightgrey;padding-top:13px;font-size:10px;">
        <div style="display:inline-block;width:200px;margin-left:5px;" >{{__('admin.product_name')}}</div>
        <div style="display:inline-block;width:33px;margin-left:5px">{{__('admin.product_color')}}</div>
        <div style="display:inline-block;width:140px;margin-left:5px;text-align: center;">取引寸</div>
        <div style="display:inline-block;width:33px;margin-left:5px">{{__('admin.product_inverse')}}</div>
        <div style="display:inline-block;width:33px;margin-left:5px">{{__('admin.product_symbol')}}</div>
        <div style="display:inline-block;width:33px;margin-left:5px">{{__('admin.product_quantity')}}</div>
        <div style="display:inline-block;width:33px;margin-left:5px">{{__('admin.product_unit')}}</div>
        <div style="display:inline-block;text-align: center;width:120px;margin-left:5px;padding-top:0px;">
            <div style="margin:0px;padding:0px">{{__('admin.product_purchage')}}</div>
            <div style="display:inline-block;text-align: center;margin:0px;width:33px;">{{__('admin.product_hanging_rate')}}</div>
            <div style="display:inline-block;text-align: center;margin:0px;width:80px;">{{__('admin.product_price')}}</div>
        </div>
        <div style="display:inline-block;text-align: center;width:120px;margin-left:5px;padding-top:0px;">
        ...
        <div style="display:inline-block;width:45px;margin-left:5px;text-align: center;">{{__('admin.product_previous_generation')}}</div>
        <div style="display:inline-block;width:33px;margin-left:5px">{{__('admin.product_tax')}}</div>
        <div style="display:inline-block;width:70px;margin-left:5px;text-align: center;">{{__('admin.product_deadline')}}</div>
    </div>
    @foreach ($data->originalContractDetail as $detail)
        <div style="font-size:8px;margin-bot:0px;margin-top:2px;{{ $detail->note == null ? 'border-bottom:1px solid lightgrey;' : 'border-left: 0px;border-right: 0px;border-bottom: 1px dashed lightgray;border-style: 1px solid dashed;'}}padding: 2px 5px;background-color:rgb(255, 255, 255)">
            <div style="display:inline-block;width:200px;vertical-align: middle;margin-left:5px;" >
                <div style="word-wrap: break-word;;color:rgb(105, 105, 105);font-size:8px;width:200px;">{{ $detail->product_code }}</div>
                <div style="word-wrap: break-word;width:200px;">{{ $detail->product_name }}</div>
                {{-- <div style="word-wrap: break-word;;color:rgb(105, 105, 105);font-size:8px;width:200px;">{{ $detail->note }}</div> --}}
            </div>
            <div style="display:inline-block;width:33px;vertical-align: middle;margin-left:5px">{{ $detail->color }}</div>
            <div style="display:inline-block;width:140px;vertical-align: middle;margin-left:5px;text-align: center;">
                <div>{{$detail->trade_size_1}}</div>
                <div>{{$detail->trade_size_2}}</div>
            </div>
            <div style="display:inline-block;width:33px;vertical-align: middle;margin-left:5px">{{ $detail->reciprocal_number }}</div>
            <div style="display:inline-block;width:33px;vertical-align: middle;margin-left:5px">{{ $detail->standard }}</div>
            <div style="display:inline-block;width:33px;vertical-align: middle;margin-left:5px">{{ number_format($detail->quantity,2) }}</div>
            <div style="display:inline-block;width:33px;vertical-align: middle;margin-left:5px">{{ $detail->unit }}</div>
            <div style="display:inline-block;width:120px;vertical-align: middle;margin-left:5px">
                <div style="display:inline-block;margin:0px;width:33px;margin-bottom: -5px;text-align:center;word-wrap: break-word;">{{number_format($detail->buy_in_discount_rate,2)}}</div>
                <div style="display:inline-block;margin:0px;width:80px;margin-bottom: -5px;text-align:center;word-wrap: break-word;">{{number_format($detail->buy_price,2)}}</div>
            </div>
            <div style="display:inline-block;width:120px;vertical-align: middle;margin-left:5px">
                <div style="display:inline-block;text-align: center;margin:vertical-align: middle;0px;width:25px;margin-bottom: -5px;word-wrap: break-word;">{{number_format($detail->sales_discount_rate,2)}}</div>
                <div style="display:inline-block;text-align: center;margin:vertical-align: middle;0px;width:80px;margin-bottom: -5px;word-wrap: break-word;">{{number_format($detail->price,2)}}</div>
            </div>
            <div style="display:inline-block;width:45px;vertical-align: middle;margin-left:5px;text-align:center;word-wrap: break-word;">{{number_format($detail->consignment,2)}}</div>
            <div style="display:inline-block;width:33px;vertical-align: middle;margin-left:5px">{{$detail->tax}}%</div>
            <div style="display:inline-block;width:70px;vertical-align: middle;margin-left:5px">{{ empty($detail->delivery_term) ? "" : date('Y/m/d',strtotime($detail->delivery_term)) }}</div>
        </div>
        @if($detail->note)
        <div style="font-size:8px;margin-bot:0px;margin-top:2px;border-bottom:1px solid lightgrey;padding: 1px 5px;background-color:rgb(255, 255, 255)">
            <div style="display:inline-block;vertical-align: middle;margin-left:5px;padding:0px;word-wrap: break-word;width:300px;" >
                {{$detail->note}}
            </div>
        </div>
        @endif
        ...
        @if($order_check == false)
            ...
            <div style="float: right;width:45%;padding: 10px 10px;">
                ...
                @php
                    ...
                    foreach ($data->originalContractDetail as $detail) {
                        $tax = $detail->tax;
                        if($detail->consignment == 0){
                            $detail->consignment = 1;
                        }
                        $price = (!empty($detail->price)) ? (float)($detail->price * $detail->quantity) : (float)($detail->quantity * $detail->sales_discount_rate * (float)($detail->consignment/100))-(float)($detail->quantity * $detail->sales_discount_rate);
